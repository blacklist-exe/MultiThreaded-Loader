...
// loading images into g_vecLoadedImages
void loadImage(std::wstring filepath)
{	
	HBITMAP loadedImage = (HBITMAP)LoadImageW(NULL, filepath.c_str(), IMAGE_BITMAP, 200, 200, LR_LOADFROMFILE);
	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
				...
				//auto ImageLoadTimer_StartFlag = std::chrono::high_resolution_clock::now();
			
            // Calls ThreadPool and pool size is customisable and then allocated within the brackets "pool(4)"
		    ThreadPool pool(4);
		    ...
		    for (int i = 0; i < 8; ++i) {
			    ...
			    pool.enqueue([i] {
					...
					// LOAD IMAGES	
				for (int imageFileNameIndex = 0;
					imageFileNameIndex < g_vecImageFileNames.size();
					++imageFileNameIndex)
			...
			for(auto&& result : results)
				...
				//NEEDED FOR ALL IMAGE FILE PATH
				for (int imageFileNameIndex = 0;
					imageFileNameIndex < g_vecImageFileNames.size();
					++imageFileNameIndex)
				{
